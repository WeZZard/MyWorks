...
前两条风险为零。第三条必须先把丑话说在前面：它用的是 X **未公开的私有 API**，没有任何官方授权——X 随时可以封掉它，而且用它自动化操作可能被官方判定为滥用，**最严重的后果是封号**。能接受这个风险再往下读，文章最后会详细介绍这条路。
...
第三，**这个 API 不能更新，也不能删**。Articles API 一共只有两个端点：建草稿、发布。内容变了只能新建草稿，旧草稿在编辑器里手动丢弃；已发布的文章也只能在 X 的界面上手动删除。所以小改动直接在 X 的编辑器里改就好，只有从 Markdown 重新同步时才值得走一遍新草稿。

## 方法三：私有 GraphQL API（高风险）

最后说第三条路。开头已经警告过，这条路用哪个工具风险都在，这里以开源的 [Kaitox](https://kaitox.ai/) 为例——它的核心库 [@kaitox/x-article](https://www.npmjs.com/package/@kaitox/x-article) 是开源的（MIT），有 Chrome 扩展、Obsidian 插件和 CLI 三种形态。

它的原理：X 网页版自己的文章编辑器背后是一套**私有的 GraphQL 接口**，Kaitox 模拟的就是这套接口——用你浏览器里已登录的会话（cookie 加上所有 x.com 网页客户端共用的那个公开 bearer token），调编辑器同款的 mutation 把 Markdown 转好的 `content_state` 提交成草稿。
...
我的态度：这条路的存在说明「Markdown → X Article」的需求是真实的，从文档看它的转换层做得也很完整。但除非你对账号安全毫不在意，否则我不推荐——方法一不花钱，方法二不违规，两条安全的路都摆在前面了。
