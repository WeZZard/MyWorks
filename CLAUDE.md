...
- `SKILL.md` is the only entry point. It resolves the surface, invokes the subagents, downloads and filters seeds, generates one candidate per seed, presents them, and on the user's pick runs the final visual checks. It holds no aesthetics, no rhetoric, no surface rules, and no check criteria.
