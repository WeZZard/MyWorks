...
- Before any art direction starts, a `title-link-reflector` gates track selection on two reader-side axes: the title-swap test (would this image fit a different title just as well?) and the visual-memory check (is the figure recognized in one glance, or must it be identified before it can be read?). A candidate that fails either is sent back to the idea-extractor with notes.
...
- `references/poster-principles.md` holds the seven tests a cover must pass — atmosphere fit, thumbnail legibility, freshness, specificity, one read, title link, common visual memory (familiar object, fresh relationship — recognized in one glance) — and where the idea comes from (not always the core argument). `idea-extractor` reads it before extracting.
