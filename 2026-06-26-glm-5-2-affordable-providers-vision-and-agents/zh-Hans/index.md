...
| 套餐 | 价格 | 使用限制 | Context Window | 速度 | 视觉支持 |
| --- | --- | --- | --- | --- | --- |
| Cursor | 付费用户免费，$0 USD/month。 | 免费期间几乎没有实际上限。 | 200K | 5/5 | 自动路由到支持视觉的模型。 |
| Devin | 付费用户 7 月 5 日前免费，$0 USD/month。 | 免费期间几乎没有实际上限。 | 200K | 未验证。 | 未验证。 |
| OpenCode Go | 首月 $5 USD/month，之后 $10 USD/month。 | $60 USD/month 使用上限。 | 1M | 5/5 | 不支持。 |
| Ollama Pro/Max | Pro $20 USD/month，Max $100 USD/month。 | 按我的使用记录估算，大约每周 3,200 个请求。 | 1M | 5/5 | 不支持。 |

如果你已经在付费使用 **Cursor**，它是最省事的入口。GLM-5.2 High 对付费用户免费，而且我没找到官方公告把它说成限时优惠。只要这一点不变，用量体感就接近无限。

**Devin** 免费期内很有吸引力。付费用户可以免费用 GLM-5.2 到 7 月 5 日，而且免费期间的用量体感也接近无限。我没有 Devin 付费账号，没法替你验证它的视觉支持。

**OpenCode Go** 便宜也快，不过 $60 USD/month 的使用上限会影响长程 agent 任务的使用体验。如果你想在 OpenCode 里用 GLM-5.2，或者想用一个低成本套餐在多个 agent 里试水，它很合适。
...
我不建议选择 `bigmodel.cn` 或 `z.ai`。它们都由智谱运营，而智谱正是开发 GLM-5.2 的团队。`bigmodel.cn` 不保证 SLA，还需要 KYC 验证。`z.ai` 的价格是 `bigmodel.cn` 的两倍。
...
使用 `bigmodel.cn` 或 `z.ai` plan 时，ZCode 会自动路由视觉理解任务。但如果走的是 Ollama Pro/Max 或 OpenCode Go，就没法这样处理。
...
官方文档就是这么写的。先用这个办法，再考虑手动改 方案 文件。
...
如果你已经为 Cursor 付费，就从它开始。以我的账号情况看，它现在用 GLM-5.2 的体验最好，因为付费用户不用额外付费，客户端也会自动处理视觉路由。

如果你主要用 CLI，而且想要更大的上下文，Ollama Pro 是现实的起点。如果你是重度用户，Ollama Max 更合理。两者都很快，都给你 1M 上下文，而且 launch 命令省掉了大部分 setup。

如果你想要最便宜的能在多个 agent 中使用的订阅，OpenCode Go 仍然有用。不过 $60 USD/month 的使用限制会让它跑重度循环时最难受。
