...
不过，即便所有的 subagent 都是用 JS 编排的，我们也完全可以用 generator 模式来实现执行引擎，并直接在主 agent 中运行：
