...
在之前的文章中，我们学习了独特定义 Swift 宏的优势和本质。文中的示例工作良好，但是我们现在可以自信而大胆地实现任何 Swift 宏了吗？

答案是否定的。

带来 Swift 宏优势的特性也会引入陷阱与缺陷从而让程序员自己给自己惹麻烦。在本文中，我想向你展示我发现的一些问题以及如何避免它们。
...
由于宏展开中涉及控制流的 `return` 语句是有意行为，所以这并不让人感到意外。但是如果我们将这个宏放在循环中呢？请看下面这个例子：
...
以上提到的意外并不是我给出的 `#unwrap` 宏扩展中唯一的潜在陷阱。这里还有一个陷阱是在宏扩展后，`bar` 变量被 `#unwarp` 宏重新绑定。让我们继续检查该宏的扩展:
...
这个宏扩展引入了 variable name shadowing，其中 `guard let bar: Int` shadow 了参数 `_ bar: Int?`。对于 `#unwrap` 来说，variable name shadowing 是无足轻重的，因为这是一种有意的行为。然而，在真实世界的编程中，`Optional` 之外的 variable name shadowing 都会被认为是一种不好的实践 - 实际上，在 Swift 中这是无法编译的。如我之前所总结，freestanding Swift macro 的宏展开涉及应用点的词法作用域共享。这使得宏展开中可能发生 variable name shadowing。以下是一个人造的例子，由于宏展开，变量名 `updater` 被 shadow 了:
...
潜在的命名冲突不仅可以在 freestanding macro 中产生，还可以在 attached macro 中产生。我们可以从之前那篇文章中`@COW` 宏的示例中了解到这一点。让我们回顾一下这个示例的宏展开：
...
这是我从苹果的 Swift Observation 和 SwiftData 宏实现中学到的命名习惯，该习惯可以保护附加宏的实现细节不被程序员意外访问。然而，这并不能保护这些成员免受其他宏带来的意外重声明或访问——因为程序员应用可能有其他宏，而这些宏添加了重复名称的成员或者误用了其他宏添加的成员。
...
在 Swift 中，一些语言结构在其超结构下是唯一的。这意味着当多个宏尝试在相同的父一级结构中生成相同的子结构时，代码会变得无法编译。例如，在属性声明中的 `get` 和 `set` accessor 中，如果我们在属性声明中添加多个 `get` accessor，那么代码将无法编译。这个错误通常在手写代码中发生，但在使用宏时需要引起注意。
...
然而，这只是对所发生的过程进行了过度简化。带有 `@COW` 宏的真实展开结果是：
...
我们可以观察到，在 `info` 属性下生成了两个 `get` 和 `set` accessor。由于 Swift 编程语言的语法只允许一个属性中有一个 `get`/`set` accessor，这个展开会导致 Swift 中的语法错误，最终使得代码无法编译。

然而，这只是狭隘的视角。通过COW宏的实际生产级实现，`get`和`set`访问器会被优化为`_read`和`_modify`，这在生产环境中可能提供更好的性能。我们可以观察到，Swift编程语言禁止程序员定义不仅名称相同而且语义相同的访问器。
...
由于我们已经学习了几种由于添加声明而导致的潜在命名冲突的案例，你可能会认为命名冲突的列表已经完结了。

但是不是这样的。命名冲突不仅可以由变量和属性的 accessor 等声明引起，还可以由对其他框架中声明的引用引起。我想向你展示一下我们如何通过重构 `@COW` 宏示例而陷入这种情况。
...
然后，我们可以通过将 `@Box` 附着到宏展开引入的 `_$storage` 属性，从而消除原地生成的 `makeStorageUniqueIfNeeded` 函数。这减少了冗余并增加了编译速度。现在这个宏的实现更加成熟了。
...
通过这些例子，我们可以了解到 Swift 宏的展开可能会改变源代码的语义。这可能导致语义冲突，最终导致展开结果无法编译通过。

## 解决方案：渐进式控制宏展开

到目前为止，我们已经列出了一系列在实现 Swift 宏时可能会遇到的典型陷阱和缺陷。乍一看，这个列表可能让你感到不知所措。然而，我发明了一种简单的方法来摆脱这些问题：渐进式控制宏展开。这个方法源自于"渐进式暴露 API 设计"和"渐进式类型化"的思想，并借鉴了从苹果 Swift Observation 和 SwiftData 的实现中提取出的一些思路。

"渐进式控制宏展开"的思想非常简单：如果没有冲突，那么程序员就不需要费力绕过冲突解决机制；如果有，那么我们必须有工具使得程序员可以用最小的努力解决冲突。

### 最大化幸运案例的概率

如果程序员在使用 Swift 宏的过程中不需要解决任何冲突，那么我们可以说程序员进入了幸运案例。为了最大化 Swift 宏用户获得预期结果的可能性，我们必须遵守以下几点：

> 条目 1：用于操作控操控的宏应该具有反映这一目的的名称。
...
通过将这个宏重命名为 `#returnIfAnyOptional`，程序员可以更容易地摆脱这样一个陷阱。
...
这种类似“保护伞”的结构也可以用于 attached macro 的宏展开，例如 `@COW` 宏的展开将声明一个嵌套的 `Storage` 类型作为存储类型。这个嵌套类型将起到前面提到的“保护伞”的作用 -- 通过类型作用域保护其成员免受重复声明的影响。
...
在将 `@Box` property wrapper 引入 `@COW` 宏展开的示例中，我们可以发现 `Box` 本身的名称可能会产生歧义，因为其他被 import 的框架可能有相同名称的声明。我们可以通过使用 fully-qualified name 来解决这个问题。假设库的名称是`COW`，那么 `Box` 的 fully-qualified name 是 `COW.Box`。
...
通过这个条目，程序员仍然不需要费力解决冲突。这个条目中描述的机制被一些 AST 转写的语言特性（如 `Equatable` 和 `Hashable`）采用 —— 如果 `Equatable` 或 `Hashable` 已经由程序员实现，编译器就不会代表程序员实现这些协议。在 Swift Observation 中，当程序员实现了 `access` 函数或 `withMutation` 函数时，也可以观察这个现象。由于这个机制是普遍存在的，学习曲线也应该是平缓的。

> 条目 5：在可以重命名声明时，为你的宏展开内容提供重命名的方式。

我们可以通过在单个类型上叠加 `@COW` 宏和 `@DictionaryLike` 宏的示例来测试此条目。这两个宏的扩展会生成两个 `_$storage` 变量。为了添加重命名机制，我们可以自然而然地想到在宏中添加一个参数：
...
但是这会破坏宏扩展中“渐进式控制”的理念：如果没有冲突，那么程序员就不需要关心解决冲突 —— 有了这个宏声明，程序员每次使用该宏都需要附加一个额外的参数。然而，你可能还不知道，我们可以通过宏重载来解决这个问题。这意味着我们可以有多个同名但具有不同“签名”的宏。
...
然而，我们可能会生成无法重命名的声明，比如 `get` 和 `set` accessor。那么接下来我们会遇到最昂贵的条目：
...
然而，由于 Swift Observation 还提供了一个附加宏称为 `@ObservationIgnored`，可以停止 `@Observable` 对应用成员带来的代码生成，因此我们可以使用这个宏来手动解决冲突。

首先，我们需要将 `name` 属性加上 `_` 前缀，并在其上附加 `@ObservationIgnored`。
...
由于这个名称可能在调试时使用，所以可读性对于理解代码的目的至关重要。这就是为什么我不推荐使用 `MacroExpansionContext.makeUniqueName(_:)` 的原因。
